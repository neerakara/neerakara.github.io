...
description: "An intro to the main optimization objects in deep learning."
categories: [Optimization]
---

Training a neural network typically involves minimizing a loss function using gradient-based optimization.
Here, let's review the basic mathematical objects behind these algorithms: gradients, Jacobians and Hessians.
...
Gradient descent steps in the opposite direction, where $\eta$ is the learning rate:
...
The Jacobian $J_\theta = \frac{\partial f_\theta(x)}{\partial \theta} \in \mathbb{R}^{k \times p}$ captures how each output logit $z_i$ changes with each parameter. It connects model outputs to parameters and is essential for computing gradients via the chain rule (Section 3):
...
The Jacobian $J_\theta$ is crucial in the computation of gradients via the chain rule,
and is central to how neural networks are trained.
Gradient descent requires $\nabla_\theta \ell(y, f_\theta(x)) \in \mathbb{R}^p$,
the gradient of the scalar loss w.r.t. model parameters.
By the chain rule, this is a vector-Jacobian product (VJP):

$$\nabla_\theta \ell = \underbrace{(\nabla_z \ell)^\top}_{\mathbb{R}^{1\times k}} \underbrace{J_\theta}_{\mathbb{R}^{k\times p}} \in \mathbb{R}^p$$
...
This section motivates and derives the widely used [Adam](https://arxiv.org/abs/1412.6980) optimizer in three steps:
(1) show why curvature matters,
(2) approximate it cheaply using the diagonal Hessian,
(3) see how Adam implements that.
...
The inverse Hessian rescales the gradient:
steps are larger in flat directions and smaller in steep ones.
Unfortunately, $H_\theta \in \mathbb{R}^{p\times p}$ is far too large to compute or invert for modern networks.
...
This gives a per-parameter learning rate, where $h_i$ estimates the curvature of parameter $i$:

$$\theta_{t+1,i} = \theta_{t,i} - \eta \, \frac{\nabla_{\theta_i}\ell}{\sqrt{h_i}}$$

For likelihood-based losses,
the diagonal of the expected Hessian can be approximated using squared gradients (see [Appendix B](#appendix-b)):
...
Adam implements the diagonal curvature idea by maintaining two running estimates in parallel:
one for direction ($m_t$, the smoothed gradient) and one for scale ($v_t$, the smoothed squared gradient):

$$
\nabla_\theta \ell \;\xrightarrow{\;\text{EMA}_{\beta_1}\;}\; \underset{\text{direction}}{m_t}
\qquad\qquad
(\nabla_\theta \ell)^2 \;\xrightarrow{\;\text{EMA}_{\beta_2}\;}\; \underset{\text{scale}}{v_t}
$$

$$m_t = \beta_1 m_{t-1} + (1-\beta_1)\nabla_\theta \ell, \qquad v_t = \beta_2 v_{t-1} + (1-\beta_2)(\nabla_\theta \ell)^2$$

The two moments are then combined into the update:

$$\theta_{t+1} = \theta_t - \eta \frac{m_t}{\sqrt{v_t}+\epsilon}$$

The denominator $\sqrt{v_t}$ acts as a diagonal curvature estimate,
scaling each parameter's update by its historical gradient magnitude.
:::
...
At first it may seem surprising that curvature (a second-order quantity)
can be estimated using gradients (a first-order quantity).
The key point is that the relationship holds *in expectation over the dataset*,
rather than at individual datapoints.

Consider the gradient of the loss with respect to a parameter $\theta_j$ for each training example:
$g_{i,j} = \frac{\partial \ell(x_i, y_i)}{\partial \theta_j}$.
If the loss surface is strongly curved along parameter $\theta_j$,
the gradient with respect to that parameter changes rapidly as $\theta_j$ varies,
which manifests as large variability in per-sample gradients across the dataset.

Now, the expected squared gradient can be decomposed as
$\mathbb{E}[g_{i,j}^2] = (\mathbb{E}[g_{i,j}])^2 + \mathrm{Var}(g_{i,j})$,
i.e. it captures both the average gradient signal and
the variability of gradients across samples.
Under the likelihood assumptions above,
this second moment corresponds to the expected curvature along that parameter.

**Caveats**

1. Exact only for negative log-likelihood losses.
2. Uses the expected Hessian, not the local Hessian at $\theta_t$.
